...
public class ClusterBuilderTest {
    ...
    public static Iterable<Object[]> data() {
        return Arrays.asList(new Object[][]{
                ...
                {"highWatermark0", Cluster.build().highWatermark(0), "highWatermark must be greater than zero"},
                {"highWatermarkNeg1", Cluster.build().highWatermark(-1), "highWatermark must be greater than zero"},
                {"lowWatermarkNeg1", Cluster.build().lowWatermark(-1), "lowWatermark must be greater than or equal to zero"},
                {"lowWatermarkLtMax", Cluster.build().lowWatermark(100).highWatermark(99), "highWatermark cannot be less than lowWatermark"},
